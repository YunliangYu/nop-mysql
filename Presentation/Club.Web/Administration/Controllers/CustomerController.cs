...
using Club.Admin.Models.ShoppingCart;
using Club.Core;
        ...
        public virtual ActionResult SendEmail(CustomerModel model)
        {
            ...
            try
            {
                if (String.IsNullOrWhiteSpace(customer.Email))
                    throw new SiteException("Customer email is empty");
                if (!CommonHelper.IsValidEmail(customer.Email))
                    throw new SiteException("Customer email is not valid");
                if (String.IsNullOrWhiteSpace(model.SendEmail.Subject))
                    throw new SiteException("Email subject is empty");
                if (String.IsNullOrWhiteSpace(model.SendEmail.Body))
                    throw new SiteException("Email body is empty");

                var emailAccount = _emailAccountService.GetEmailAccountById(_emailAccountSettings.DefaultEmailAccountId);
                if (emailAccount == null)
                    emailAccount = _emailAccountService.GetAllEmailAccounts().FirstOrDefault();
                if (emailAccount == null)
                    throw new SiteException("Email account can't be loaded");
        ...
        public virtual ActionResult SendPm(CustomerModel model)
        {
            ...
            try
            {
                if (!_forumSettings.AllowPrivateMessages)
                    throw new SiteException("Private messages are disabled");
                if (customer.IsGuest())
                    throw new SiteException("Customer should be registered");
                if (String.IsNullOrWhiteSpace(model.SendPm.Subject))
                    throw new SiteException("PM subject is empty");
                if (String.IsNullOrWhiteSpace(model.SendPm.Message))
                    throw new SiteException("PM message is empty");
