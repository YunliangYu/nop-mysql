DROP PROCEDURE IF EXISTS `club_splitstring_to_table`;

CREATE PROCEDURE `club_splitstring_to_table`
(
    IN string LONGTEXT,
    IN delim CHAR(1)
)
BEGIN
    DROP TEMPORARY TABLE IF EXISTS club_splitstring_to_table_TempTable;
    CREATE TEMPORARY TABLE club_splitstring_to_table_TempTable
    (
        `data` LONGTEXT
    );
    ...
    WHILE @start < LENGTH(string) + 1 DO
        ...
        INSERT INTO club_splitstring_to_table_TempTable (data) 
...
DROP PROCEDURE IF EXISTS `club_getnotnullnotempty`;

CREATE PROCEDURE `club_getnotnullnotempty`
(
    IN p1 LONGTEXT, 
    IN p2 LONGTEXT,
    OUT res LONGTEXT
...
DROP PROCEDURE IF EXISTS `club_getprimarykey_indexname`;

CREATE PROCEDURE `club_getprimarykey_indexname`
(
...
(
	...
	IN MarkedAsNewOnly	bit,
	IN FeaturedProducts	bit,
	IN ProductTagId		int,
...
BEGIN
	...
	IF FeaturedProducts IS NULL THEN
		SET FeaturedProducts = NULL;
	END IF;
...
--          Select @TempKeywords;';
  ...
  CALL club_splitstring_to_table(CategoryIds, ',');
	INSERT INTO FilteredCategoryIds_TempTable (CategoryId)
	SELECT (data + 0) FROM club_splitstring_to_table_TempTable;
  ...
  CALL club_splitstring_to_table(FilteredSpecs, ',');
  ...
  SELECT COUNT(1) FROM FilteredSpecs_TempTable into @SpecAttributesCount;
	...
	CALL club_splitstring_to_table(AllowedCustomerRoleIds, ',');
...
DROP PROCEDURE IF EXISTS CategoryLoadAllPaged;
CREATE PROCEDURE `CategoryLoadAllPaged`
(
    IN ShowHidden           bit,
    IN Name                 nvarchar(4000),
    IN StoreId              int,
    IN CustomerRoleIds	    nvarchar(4000),
    IN PageIndex			int,
	IN PageSize			    int,
    out TotalRecords		int
)
...
BEGIN
	...
	END IF;
   	...
   	SET CustomerRoleIds = COALESCE(CustomerRoleIds, '');	
	Drop TEMPORARY TABLE IF EXISTS FilteredCustomerRoleIds;
	CREATE TEMPORARY TABLE #FilteredCustomerRoleIds
	(
		CustomerRoleId int not null
	) ENGINE = MEMORY;
    
    CALL club_splitstring_to_table(CustomerRoleIds, ',');
	INSERT INTO FilteredCustomerRoleIds (CustomerRoleId)
	SELECT (data + 0) FROM club_splitstring_to_table_TempTable;	
    SELECT COUNT(1) FROM FilteredCustomerRoleIds;


    --ordered categories
    CREATE TEMPORARY TABLE #OrderedCategoryIds
	(
		[Id] int not null auto_increment,
