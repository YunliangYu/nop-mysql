...
using Club.Services.Authentication.External;
using Club.Services.Blogs;
...
using Club.Services.ExportImport;
using Club.Services.Forums;
using Club.Services.Helpers;
using Club.Services.Infrastructure;
using Club.Services.Installation;
using Club.Services.Localization;
using Club.Services.Logging;
using Club.Services.Media;
using Club.Services.Messages;
using Club.Services.News;
using Club.Services.Orders;
using Club.Services.Payments;
using Club.Services.Polls;
...
using Club.Web.Framework.Themes;
using Club.Web.Framework.UI;
    ...
    public class DependencyRegistrar : IDependencyRegistrar
    {
        ...
        public virtual void Register(ContainerBuilder builder, ITypeFinder typeFinder, SiteConfig config)
        {
            ...
            if (dataProviderSettings != null && dataProviderSettings.IsValid())
            {
                ...
                builder.Register<IDbContext>(c => new SiteObjectContext(dataProviderSettings.DataConnectionString)).InstancePerLifetimeScope();
            }
            else
            {
                builder.Register<IDbContext>(c => new SiteObjectContext(dataSettingsManager.LoadSettings().DataConnectionString)).InstancePerLifetimeScope();
            }
            ...
            if (config.RedisCachingEnabled)
            {
                ...
                builder.RegisterType<RedisCacheManager>().As<ICacheManager>().Named<ICacheManager>("club_cache_static").InstancePerLifetimeScope();
            }
            else
            {
                builder.RegisterType<MemoryCacheManager>().As<ICacheManager>().Named<ICacheManager>("club_cache_static").SingleInstance();
            }
            builder.RegisterType<PerRequestCacheManager>().As<ICacheManager>().Named<ICacheManager>("club_cache_per_request").InstancePerLifetimeScope();
