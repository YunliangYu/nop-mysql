...
namespace Club.Services.Directory
{
    ...
    public partial class StateProvinceService : IStateProvinceService
    {
        ...
        /// </remarks>
        private const string STATEPROVINCES_ALL_KEY = "Club.stateprovince.all-{0}-{1}-{2}";
        /// <summary>
        /// Key pattern to clear cache
        /// </summary>
        private const string STATEPROVINCES_PATTERN_KEY = "Club.stateprovince.";
        ...
        public StateProvince GetStateProvinceByStateProvinceName(string stateProvinceName)
        {
            if (string.IsNullOrEmpty(stateProvinceName))
                return null;

            var query = from d in _stateProvinceRepository.Table
                        where d.Name == stateProvinceName
                        orderby d.Id
                        select d;
