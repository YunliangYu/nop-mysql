...
using Club.Services.Shipping;
using Club.Services.Tax;
    ...
    public partial class OrderProcessingService : IOrderProcessingService
    {
        ...
        protected virtual PlaceOrderContainter PreparePlaceOrderDetails(ProcessPaymentRequest processPaymentRequest)
        {
            ...
            if (details.Customer.IsGuest() && !_orderSettings.AnonymousCheckoutAllowed)
                throw new SiteException("Anonymous checkout is not allowed");
            ...
            if (details.Customer.BillingAddress == null)
                throw new SiteException("Billing address is not provided");
            ...
            if (details.BillingAddress.Country != null && !details.BillingAddress.Country.AllowsBilling)
                throw new SiteException(string.Format("Country '{0}' is not allowed for billing", details.BillingAddress.Country.Name));
            ...
            if (!details.Cart.Any())
                throw new SiteException("Cart is empty");

            //validate the entire shopping cart
            var warnings = _shoppingCartService.GetShoppingCartWarnings(details.Cart, details.CheckoutAttributesXml, true);
            if (warnings.Any())
                throw new SiteException(warnings.Aggregate(string.Empty, (current, next) => string.Format("{0}{1};", current, next)));
            ...
            foreach (var sci in details.Cart)
            {
                ...
                if (sciWarnings.Any())
                    throw new SiteException(sciWarnings.Aggregate(string.Empty, (current, next) => string.Format("{0}{1};", current, next)));
            }
            ...
            if (!ValidateMinOrderSubtotalAmount(details.Cart))
            {
                ...
                throw new SiteException(string.Format(_localizationService.GetResource("Checkout.MinOrderSubtotalAmount"),
            ...
            if (!ValidateMinOrderTotalAmount(details.Cart))
            {
                ...
                throw new SiteException(string.Format(_localizationService.GetResource("Checkout.MinOrderTotalAmount"),
            ...
            if (details.Cart.RequiresShipping())
            {
                ...
                else
                {
                    if (details.Customer.ShippingAddress == null)
                        throw new SiteException("Shipping address is not provided");
                    ...
                    if (details.ShippingAddress.Country != null && !details.ShippingAddress.Country.AllowsShipping)
                        throw new SiteException(string.Format("Country '{0}' is not allowed for shipping", details.ShippingAddress.Country.Name));
                }
            ...
            if (!orderShippingTotalInclTax.HasValue || !orderShippingTotalExclTax.HasValue)
                throw new SiteException("Shipping total couldn't be calculated");
            ...
            if (!orderTotal.HasValue)
                throw new SiteException("Order total couldn't be calculated");
            ...
            if (details.IsRecurringShoppingCart)
            {
                ...
                if (!string.IsNullOrEmpty(recurringCyclesError))
                    throw new SiteException(recurringCyclesError);
        ...
        protected virtual PlaceOrderContainter PrepareRecurringOrderDetails(ProcessPaymentRequest processPaymentRequest)
        {
            ...
            if (details.InitialOrder.BillingAddress == null)
                throw new SiteException("Billing address is not available");
            ...
            if (details.InitialOrder.ShippingStatus != ShippingStatus.ShippingNotRequired)
            {
                ...
                if (!details.PickUpInStore)
                {
                    if (details.InitialOrder.ShippingAddress == null)
                        throw new SiteException("Shipping address is not available");
            ...
            try
            {
                ...
                if (!skipPaymentWorkflow)
                {
                    ...
                    if (paymentMethod == null)
                        throw new SiteException("Payment method couldn't be loaded");

                    //ensure that payment method is active
                    if (!paymentMethod.IsPaymentMethodActive(_paymentSettings))
                        throw new SiteException("Payment method is not active");
                    ...
                    if (details.IsRecurringShoppingCart)
                    {
                        ...
                        switch (_paymentService.GetRecurringPaymentType(processPaymentRequest.PaymentMethodSystemName))
                        {
                            case RecurringPaymentType.NotSupported:
                                throw new SiteException("Recurring payments are not supported by selected payment method");
                            case RecurringPaymentType.Manual:
                            case RecurringPaymentType.Automatic:
                                processPaymentResult = _paymentService.ProcessRecurringPayment(processPaymentRequest);
                                break;
                            default:
                                throw new SiteException("Not supported recurring payment type");
                        }
                    }
                    else
                        //standard cart
                ...
                if (processPaymentResult == null)
                    throw new SiteException("processPaymentResult is not available");
        ...
        public virtual IEnumerable<string> ProcessNextRecurringPayment(RecurringPayment recurringPayment, ProcessPaymentResult paymentResult = null)
        {
            ...
            try
            {
                if (!recurringPayment.IsActive)
                    throw new SiteException("Recurring payment is not active");

                var initialOrder = recurringPayment.InitialOrder;
                if (initialOrder == null)
                    throw new SiteException("Initial order could not be loaded");

                var customer = initialOrder.Customer;
                if (customer == null)
                    throw new SiteException("Customer could not be loaded");

                var nextPaymentDate = recurringPayment.NextPaymentDate;
                if (!nextPaymentDate.HasValue)
                    throw new SiteException("Next payment date could not be calculated");
        ...
        public virtual void CancelOrder(Order order, bool notifyCustomer)
        {
            ...
            if (!CanCancelOrder(order))
                throw new SiteException("Cannot do cancel for order.");
        ...
        public virtual IList<string> Capture(Order order)
        {
            ...
            if (!CanCapture(order))
                throw new SiteException("Cannot do capture for order.");
        ...
        public virtual void MarkOrderAsPaid(Order order)
        {
            ...
            if (!CanMarkOrderAsPaid(order))
                throw new SiteException("You can't mark this order as paid");
        ...
        public virtual IList<string> Refund(Order order)
        {
            ...
            if (!CanRefund(order))
                throw new SiteException("Cannot do refund for order.");
        ...
        public virtual void RefundOffline(Order order)
        {
            ...
            if (!CanRefundOffline(order))
                throw new SiteException("You can't refund this order");
        ...
        public virtual IList<string> PartiallyRefund(Order order, decimal amountToRefund)
        {
            ...
            if (!CanPartiallyRefund(order, amountToRefund))
                throw new SiteException("Cannot do partial refund for order.");
        ...
        public virtual void PartiallyRefundOffline(Order order, decimal amountToRefund)
        {
            ...
            if (!CanPartiallyRefundOffline(order, amountToRefund))
                throw new SiteException("You can't partially refund (offline) this order");
        ...
        public virtual IList<string> Void(Order order)
        {
            ...
            if (!CanVoid(order))
                throw new SiteException("Cannot do void for order.");
        ...
        public virtual void VoidOffline(Order order)
        {
            ...
            if (!CanVoidOffline(order))
                throw new SiteException("You can't void this order");
